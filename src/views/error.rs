...
pub(super) fn ErrorView() -> Element {
    ...
    let suggestion = if error.suggestion.trim().is_empty() {
        ...
        section { class: "screen-grid",
            div { class: "main-column",
                div { class: "attention-card",
                    ...
                    div { class: "action-row",
                        button {
                            ...
                            "{i18n::t(\"fix_and_retry\")}"
                        }
                        button {
                            class: "secondary-button",
                            onclick: move |_| ctx.backend.send(BackendAction::RefreshDependencies),
                            "{i18n::t(\"recheck_dependencies\")}"
                        }
                        button {
                            class: "ghost-button",
                            onclick: move |_| ctx.screen.set(Screen::Settings),
                ...
                div { class: "debug-panel",
                    button {
                        class: "text-button",
                        onclick: move |_| ctx.show_debug.set(!show_debug),
                        if show_debug {
                            "{i18n::t(\"hide_debug_output\")}"
                        } else {
                            "{i18n::t(\"show_debug_output\")}"
                        }
                    }
                    if show_debug {
                        pre { "{error.debug}" }
