    ...
    rsx! {
        section { class: "screen-grid",
            div { class: "main-column",
                div { class: "list-header",
                    ...
                    div { class: "table-tools",
                        button {
                            class: "ghost-button",
                            disabled: completed == 0 || busy,
                            onclick: move |_| clear_completed(ctx),
                            "{i18n::t(\"clear_completed\")}"
                        }
                        button {
                            class: "ghost-button",
                            onclick: move |_| ctx.screen.set(Screen::Home),
                            "{i18n::t(\"add_new_url\")}"
                        }
                        button {
                ...
                if jobs.is_empty() {
                    EmptyState {
                        title: i18n::t("empty_queue_title"),
                        message: i18n::t("empty_queue_message"),
                    }
                } else {
            ...
            aside { class: "side-panel quiet-panel",
                ...
                RangeSetting {
                    label: i18n::t("parallel_downloads"),
                ...
                button {
                    ...
                    onclick: move |_| ctx.backend.send(BackendAction::RetryFailed { settings: ctx.settings() }),
                    "{i18n::t(\"retry_failed\")}"
                }
            }
        }
