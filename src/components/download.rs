...
pub(crate) fn PresetButton(index: usize, preset: Preset) -> Element {
    ...
    let name = localized_preset_name(&preset.name);
    let detail = localized_preset_detail(&detail);
...
pub(crate) fn InspectorSettings() -> Element {
    ...
    let selected_format = localized_format_label(&(ctx.selected_format)());
    ...
    rsx! {
        ...
        StatBlock { label: i18n::t("quality"), value: selected_format }
    ...
    rsx! {
        div { class: "{row_class}",
            input {
                ...
                onchange: move |_| toggle_analysis_item(ctx, index),
            }
...
pub(crate) fn DownloadRow(job: DownloadJob) -> Element {
    ...
    let step = localized_job_step(&job.step);
    let eta = i18n::t("eta");
    ...
    rsx! {
        div { class: "{class}",
            ThumbnailBlock {
                ...
                fallback: job.status.label().to_string(),
            }
            div { class: "download-main",
                ...
                div { class: "card-actions",
                    button { onclick: move |_| copy_command_to_input(ctx, job.command_display.clone()), "{i18n::t(\"use_command\")}" }
                    if job.status == JobStatus::Completed {
                        button { onclick: move |_| reveal_output(&job.output_hint), "{i18n::t(\"reveal\")}" }
                    }
                }
            }
        }
    ...
    rsx! {
        div { class: "library-card",
            ThumbnailBlock {
                ...
                src: job.thumbnail.clone(),
                fallback: job.download_type.label().to_string(),
            }
            div { class: "library-info",
                ...
                div { class: "card-actions library-actions",
                    button { onclick: move |_| reveal_output(&output_hint), "{i18n::t(\"reveal\")}" }
                    button { onclick: move |_| requeue_job(ctx, requeue.clone()), "{i18n::t(\"re_download\")}" }
                }
            }
        }
    }
